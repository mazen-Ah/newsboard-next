...
import { getAllArticles } from "./newsService";

export const transformNews = (article: any, type: "guardian"| "newsapi"): Article => {
  if (type === "guardian") {
    return {
      id: article.webUrl, 
      source: "The Guardian",
...
export const getGuardianArticleById = async (id: string): Promise<Article | null> => {
    try {
  ...
  export const getNewsApiArticleByTitle = async (title: string): Promise<Article | null> => {
    try {
